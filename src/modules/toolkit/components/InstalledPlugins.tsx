...
export function InstalledPlugins() {
  const t = useT();
  const { installed, uninstall, toggle, serverPort } = usePluginStore();
  ...
  return (
    <div className="space-y-2">
      {installed.map((plugin) => {
        const logoUrl = getPluginLogoUrl(plugin, serverPort);
        return (
          <div key={plugin.id} className="flex items-center gap-3 p-3 border rounded-lg bg-card">
            <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center text-lg shrink-0 overflow-hidden">
