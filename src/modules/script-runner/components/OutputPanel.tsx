...
export function OutputPanel() {
  ...
  // Keep last finished output to avoid fallback-to-history duplication
  const lastOutputRef = useRef<{ stdout: string; stderr: string; exitCode: number | null; durationMs: number; timestamp: number } | null>(null);

  const { runningProcess, executionHistory } = store;

  // When a process finishes, capture its output
  const prevRunning = useRef(runningProcess);
  useEffect(() => {
    if (prevRunning.current && !runningProcess && executionHistory[0]) {
      lastOutputRef.current = {
        stdout: executionHistory[0].stdout,
        stderr: executionHistory[0].stderr,
        exitCode: executionHistory[0].exitCode,
        durationMs: executionHistory[0].durationMs,
        timestamp: executionHistory[0].timestamp,
      };
    }
    prevRunning.current = runningProcess;
  }, [runningProcess, executionHistory]);
  ...
  const renderAnsi = (stdout: string, stderr: string) => {
    const html = ansiToHtml(stdout) + (stderr ? `<span style="color:#e06c75">${ansiToHtml(stderr)}</span>` : '');
  ...
  return (
    <div className="flex flex-col h-full bg-card">
      <div className="flex items-center gap-1 px-2 py-1 border-b border-border text-xs">
        ...
        {tab === 'output' && (
          <>
            <button onClick={() => { store.clearOutput(); lastOutputRef.current = null; }} className="px-1.5 hover:text-foreground text-muted-foreground cursor-pointer" title={t('scriptRunner.clearOutput')}>🗑</button>
      ...
      {tab === 'output' ? (
        <div className="flex-1 flex flex-col min-h-0">
          <pre ref={outputRef} className="flex-1 overflow-auto p-2 font-mono text-xs whitespace-pre-wrap">
            {currentOutput ? renderAnsi(currentOutput.stdout, currentOutput.stderr) : (
              <span className="text-muted-foreground">{t('scriptRunner.selectOrCreate')}</span>
            )}
          </pre>
          {currentOutput && !runningProcess && currentOutput.exitCode !== null && (
          ...
          {selectedHistory ? (
            <div className="flex flex-col h-full">
              ...
              <pre className="flex-1 overflow-auto p-2 font-mono text-xs whitespace-pre-wrap">
                {renderAnsi(selectedHistory.stdout, selectedHistory.stderr)}
              </pre>
            </div>
